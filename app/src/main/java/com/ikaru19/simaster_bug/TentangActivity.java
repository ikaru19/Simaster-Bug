...
public class TentangActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        webView = findViewById(R.id.wvTentang);
        apiService = ServiceGenerator.createService(ApiService.class);
        getData();
    }

    private void getData(){
        final LottieLoading lottieLoading = new LottieLoading(TentangActivity.this);
        lottieLoading.show();
        Call<List<ArtikelV2>> artikelCall = apiService.getInfo();
        artikelCall.enqueue(new Callback<List<ArtikelV2>>() {
            ...
            public void onResponse(Call<List<ArtikelV2>> call, Response<List<ArtikelV2>> response) {
                ...
                if (rawArtikels == null || rawArtikels.isEmpty() ){
                    Toast.makeText(TentangActivity.this,"Tidak Ada Data",Toast.LENGTH_SHORT).show();
                }else{
